...
doBase = false;
doRuns = [0 0 0 0];
RunNum = 1000; % how many random sims to do?
...
Dir = 'runs_RangeSim_Feb2022_n100/';

% Baseline run:
if doBase
    
   % for i = 1:RunNum
    [Nf_tmp,Nm_tmp, dx]=epigenetic_model('Baseline',1,1,0,'EE2',RandSim,RangeSim,1);
    Nf(:,:) = Nf_tmp;
    Nm(:,:) = Nm_tmp;
...
for c = 1:(length(Chems))
    ...
    Nt = Nt(:);
    Nt = Nt/Nt(70);  %scale to t = 70 NOW 24
  ...
  %  Nt_low = quantile(Nt,0.05,2);
    plot(Time,Nt,'k-','linewidth',2)
   % plot(Time,Nt_low,'k-','linewidth',2)
   % plot(Time,Nt_up,'k-','linewidth',2)
    text(4.1,Nt(end),Labs{1},'color','k','fontsize',12)

    
       
    for tt  =1:3%length(Types2)   
    fname = strcat(Dir,Chems{c},'_',Types{t},'_',Types2{tt},'.mat');
    load(fname,'Nf','Nm','dx')
    Nt = squeeze(sum(Nf,1))*dx + squeeze(sum(Nm,1))*dx; % now these are time x rep
   ...
   % Nt = Nt./repmat(Nt(70,:),[size(Nt,1),1]); % scale to t = 70
    ...
    % the next two lines are a hack when baseline is run at a different
    % time
   % T = size(Nt,1);
   % Time = (1:T)/6 - 24/6;
%   plot(Time,Nt,'k-','linewidth',2,'color',Cols(tt,:),'linestyle',Lty{tt});
    ...
    text(6.1,Nt_med(end),Labs{tt+1},'color',Cols(tt,:),'fontsize',12)
